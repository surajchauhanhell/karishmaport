...
try {
  ...
  const user = { id: '00000000-0000-0000-0000-000000000010', email: 'admin@example.test', aud: 'authenticated', role: 'authenticated', app_metadata: {}, user_metadata: {}, created_at: new Date().toISOString() };
  ...
  let member = true, valid = true, rpcFailure = false, protectedReads = 0;
  await page.route('https://fixture.supabase.co/**', async route => {
    const path = new URL(route.request().url()).pathname;
    const reply = (body, status = 200) => route.fulfill({ status, contentType: 'application/json', body: JSON.stringify(body) });
    if (path.endsWith('/token')) return reply({ access_token: token, refresh_token: 'fixture', expires_in: 3600, token_type: 'bearer', user });
    if (path.endsWith('/user')) return valid ? reply(user) : reply({ message: 'Invalid JWT', code: 'bad_jwt' }, 401);
    if (path.endsWith('/logout')) return reply({});
    if (path.endsWith('/rpc/is_admin')) return rpcFailure ? reply({ message: 'Unavailable' }, 500) : reply(member);
    if (path.includes('/rest/v1/')) { if (!path.endsWith('/creator_settings')) protectedReads++; return reply([]); }
    return reply({});
  });
  const login = async () => {
  ...
  console.log('Auth contracts passed: anonymous denial, return path, membership revocation, non-admin denial, verification failure/retry, invalid session, sign-out.');

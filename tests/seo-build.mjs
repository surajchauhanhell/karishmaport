...
for (const [route, meta] of Object.entries(seo.pages)) {
  ...
  assert.equal(person['@id'], seo.origin + '/#person');
  assert.ok(person.alternateName.includes('itskarishma'));
  assert.ok(person.sameAs.some((url) => url.includes('instagram.com/')));
  assert.ok(person.sameAs.some((url) => url.includes('youtube.com/')));
  assert.ok(graph.some((node) => node['@type'] === 'WebSite' && node.name === 'Its Karishma'));
  ...
  assert.ok(html.includes('<main id="main"'), `Missing initial content: ${route}`);
  assert.ok(html.includes('<h1>'), `Missing initial heading: ${route}`);
  assert.ok(html.includes('rel="me noopener noreferrer" href="https://www.youtube.com/'));
...
assert.ok(
  locations.every((url) => url.startsWith(seo.origin + '/') && !/\/(admin|go)(\/|$)/.test(url)),
);
assert.ok(
  (await fs.readFile('dist/robots.txt', 'utf8')).includes(`Sitemap: ${seo.origin}/sitemap.xml`),
);
const config = JSON.parse(await fs.readFile('vercel.json', 'utf8'));
assert.ok(
  config.routes.some(
    (r) => r.src === '/(admin|go)(/.*)?' && r.headers['X-Robots-Tag'] === 'noindex, nofollow',
  ),
);
assert.ok(config.routes.some((r) => r.dest === '/404.html' && r.status === 404));
assert.ok((await fs.readFile('dist/404.html', 'utf8')).includes('noindex,follow'));
assert.equal(
  (await fs.readFile('dist/googlef0b545d4da8f015e.html', 'utf8')).trim(),
  'google-site-verification: googlef0b545d4da8f015e.html',
);
console.log(
  `SEO checks passed: ${locations.length} canonical URLs, unique metadata, structured data, sitemap and private-route indexing rules.`,
);
